...
class CustomTaintAnalysisKeySpec(TaintAnalysis):
    def vulnerable_path_found_callback(
        ...
        if (
            ...
            if isinstance(last_invocation_params[2], str):
                ...
                self.vulnerabilities[
                    ...
                    f'constant salt "{last_invocation_params[2]}" parameter passed '
                    "to PBEKeySpec",
                    " --> ".join(
        ...
        if (
            ...
            if isinstance(last_invocation_params[1], str):
                ...
                self.vulnerabilities[
                    ...
                    f'constant salt "{last_invocation_params[1]}" parameter passed '
                    "to PBEParameterSpec",
                    " --> ".join(
                        f"{p.class_name}->{p.name}{p.descriptor}" for p in full_path
                    ),
