...
class CustomTaintAnalysis(TaintAnalysis):
    ...
    def vulnerable_path_found_callback(
        ...
        if (
            full_path[-1].get_method().get_name() == "myPid"
            and full_path[-1].get_method().get_descriptor() == "()I"
            and full_path[-1].get_method().get_class_name() == "Landroid/os/Process;"
        ):
            self.my_pid = True

        elif (
            full_path[-1].get_method().get_name() == "myUid"
            and full_path[-1].get_method().get_descriptor() == "()I"
            and full_path[-1].get_method().get_class_name() == "Landroid/os/Process;"
        ):
            self.my_uid = True
...
class CheckPermission(categories.ICodeVulnerability):
    def __init__(self):
    ...
    def check_vulnerability(
        ...
        try:
            ...
            target_method: List[MethodAnalysis] = [
                dx.get_method_analysis_by_name("Landroid/os/Process;", "myPid", "()I"),
                dx.get_method_analysis_by_name("Landroid/os/Process;", "myUid", "()I"),
            ]

            target_method.extend(
                [
                    dx.get_method_analysis_by_name(
                        caller, "checkPermission", "(Ljava/lang/String; I I)I"
                    )
